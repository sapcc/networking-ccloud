...
class EOSGNMIClient:
    ...
    class PATHS:
        ...
        EVPN_INSATNCE = "arista/eos/arista-exp-eos-evpn:evpn/evpn-instances/evpn-instance[name={vlan}]"
    ...
    def _run_method(self, method, *args, retries=3, **kwargs):
        try:
            ...
            cmd = f"{method}("
            if args:
                cmd += ", ".join(map(json.dumps, args))
            if kwargs:
                if args:
                    cmd += " "
                cmd += ", ".join(f"{k}={json.dumps(v)}" for k, v in kwargs.items())
            cmd += ")"
    ...
    def get(self, *args, unpack=True, single=True, **kwargs):
        data = self._run_method("get", *args, **kwargs)
        if data and unpack:
            data = data['notification']
            if single:
                data = data[0]['update'][0]['val']
        return data

    def set(self, *args, retries=3, **kwargs):
        return self._run_method("set", *args, **kwargs)
        # if isinstance(ret, grpc._channel._InactiveRpcError):
        #     if ret.code() in (grpc.StatusCode.UNAVAILABLE, grpc.StatusCode.FAILED_PRECONDITION) \
        #             and retries > 0:
        #         time.sleep(0.5)
        #         return self.set(*args, retries=retries - 1, **kwargs)
        #     raise ret
        # return ret
...
class EOSSwitch(SwitchBase):
    ...
    def _make_vlan_config(self, config_req: EOSSetConfig, vlans: Optional[List[agent_msg.Vlan]], operation: Op) -> None:
        ...
        if operation in (Op.add, Op.replace):
            ...
            vlan_cfg = (EOSGNMIClient.VLANS, {'vlan': wanted_vlans})
                ...
                for curr_map in curr_maps:
                    for os_map in vxlan_maps:
                        if os_map.vni == curr_map.vni and os_map.vlan != curr_map.vlan:
                            ...
                            del_map = EOSGNMIClient.PATHS.VXMAP_VLAN.format(vlan=curr_maps.vlan)
                ...
                for os_map in vxlan_maps:
                    if curr_map.vlan == os_map.vlan:
                        if curr_map.vni == os_map.vni:
                            ...
                            LOG.warning("Not deleting vlan %s from switch, as it points to vni %s "
                                        "(delete requested vni %s)",
                                        curr_map.vlan, curr_map.vni, os_map.vni)
                        break
                else:
                    LOG.warning("VLAN %s not found on switch, not deleting it", curr_map.vlan)

    def get_bgp_vlan_config(self) -> List[agent_msg.BGPVlan]:
        # get potential bgw info per device from
        bgp_vlans = []
    ...
    def get_bgp_config(self) -> agent_msg.BGP:
        bgp_data = self.api.get(EOSGNMIClient.PATHS.PROTO_BGP)['openconfig-network-instance:bgp']
        bgp_asn = bgp_data['global']['config']['as']
    ...
    def _make_bgp_config(self, config_req: EOSSetConfig, bgp: Optional[agent_msg.BGP], operation: Op) -> None:
        ...
        if operation in (Op.add, Op.replace):
            ...
            for bgp_vlan in bgp.vlans:
                delete_req = EOSGNMIClient.PATHS.EVPN_INSTANCES.format(vlan=bgp_vlan.vlan)
                config_req.delete.append(delete_req)

    # FIXME: typehint
    def get_ifaces_config(self, as_dict=False) -> List[agent_msg.IfaceConfig]:
        ifaces = []
        ...
        for data in self.api.get(EOSGNMIClient.PATHS.IFACES)['openconfig-interfaces:interface']:
            ...
            if 'openconfig-if-aggregate:aggregation' in data:
                ...
                if not data_pc_cfg.get('arista-intf-augments:mlag') and not data['name'].startswith("Port-Channel"):
                    LOG.debug("Switch %s LACP iface %s has no mlag id and doesn't start with Port-Channel, skipping it",
                              self.name, data['name'])
                    continue
            ...
            if data_vlans and 'config' in data_vlans:
                ...
                if 'trunk-vlans' in data_vlans['config']:
                    vlans = []
    ...
    def _make_ifaces_config(self, config_req: EOSSetConfig, ifaces: Optional[List[agent_msg.IfaceConfig]],
        ...
        if operation in (Op.add, Op.replace):
            ...
            def calc_delete_range(all_ifaces, iface_name, iface_cfg):
                ...
                return list(set(ifaces[iface_name].trunk_vlans) - set(iface_cfg.trunk_vlans))

            all_ifaces_cfg = self.get_ifaces()
            for iface in ifaces or []:
                normal_ifaces = []
                if iface.portchannel_id is not None:
                    if iface.native_vlan:
