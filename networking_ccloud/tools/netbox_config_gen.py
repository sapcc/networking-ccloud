...
class ConfigGenerator:
    ...
    switch_name_re = re.compile(r"^(?P<region>\w{2}-\w{2}-\d)-sw(?P<switchgroup_id>(?P<az>\d)"
                                ...
                                r"(?P<leaf>[ab])(?:-(?P<role>[a-z]+(?P<bb_no>[0-9]+)?))$")
    region = "qa-de-2"
    leaf_role = "evpn-leaf"
    ...
    netbox_cluster_metagroup_handlers = {
        'cc-vsphere-prod': 'make_vpod_metagroup'
    ...
    def parse_ccloud_switch_number_resources(self, device_name: str) -> Dict[str, int]:
        ...
        return dict(
            ...
            bb_no=int(m.group('bb_no')) if m.group('bb_no') else 0,
            switchgroup_id=int(m.group('switchgroup_id'))
        )

    def get_switchgroup_attributes(self, devices: List[NbRecord]) -> Dict[str, str]:

        def get_matching_slugs(tags: List[NbRecord], mapping: Dict[str, str]) -> Set[str]:
            slugs = {x.slug for x in tags}
            return {mapping[x] for x in slugs.intersection(mapping.keys())}
        ...
        for device in devices:
            ...
            # roles
            pod_roles.update(get_matching_slugs(device.tags, self.pod_roles))
        ...
        if pod_roles == {c_const.DEVICE_TYPE_BORDER, c_const.DEVICE_TYPE_TRANSIT}:
            attributes['pod_role'] = c_const.DEVICE_TYPE_BORDER_AND_TRANSIT
        else:
    ...
    def get_switchgroup_name(self, role: str, pod: int, switchgroup_no: int, az_no: int,
                             bb_no: Optional[int], **kwargs):
        if role in {c_const.SWITCHGROUP_ROLE_NETPOD, c_const.SWITCHGROUP_ROLE_APOD, c_const.SWITCHGROUP_ROLE_STPOD,
            ...
            if bb_no == 0:
                ...
                raise ConfigSchemeException('has no bb_no')
            return f'{role}{bb_no:03d}'
    ...
    def get_switchgroups(self, asn_region: int, switches: List[NbRecord],
        ...
        for switchgroup_id, group_switches in groupby(sorted_switches, key=sorter):
            ...
            swg_attributes = self.get_switchgroup_attributes(group_switches)
            members = []
            ...
            l3_data = self.get_l3_data(asn_region, **numbered_resources)
            switchgroup_name = self.get_switchgroup_name(role=swg_attributes['pod_role'], **numbered_resources)
            switchgroup = conf.SwitchGroup(name=switchgroup_name, members=members, availability_zone=swg_attributes['az'],
    ...
    def cluster_is_valid(self, cluster) -> bool:
        ...
        if not cluster.type:
            ...
            return False
        if cluster.type.slug not in self.netbox_cluster_metagroup_handlers.keys():
    ...
    def make_vpod_metagroup(self, cluster: NbRecord, member: NbRecord) -> Optional[conf.Hostgroup]:
        ...
        return conf.Hostgroup(binding_hosts=[binding_host], metagroup=True,
                              role=conf.HostgroupRole.vpod, members=[member.name])
    ...
    def get_metagroups(self, connected_devices: Set[NbRecord]) -> List[conf.Hostgroup]:
        ...
        for device in connected_devices:
            ...
            if not metagroup:
                ...
                metagroup = getattr(self, self.netbox_cluster_metagroup_handlers[cluster.type.slug])(cluster, device)
                if not metagroup:
    ...
    def generate_config(self, region: str, switch_user: str, switch_password: str):
        config = {
            'switches': [],
            'switchgroups': [],
            'hostgroups': [],
        ...
        nb_switches = list(self.netbox.dcim.devices.filter(region=region, role=self.leaf_role, status='active'))
        ...
        # FIXME: sort hostgroups by binding host/cluster
...
def main():
    ...
    cfg = cfggen.generate_config(args.region, args.switch_user, args.switch_password)
