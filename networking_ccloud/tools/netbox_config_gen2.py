...
RESPONSIBLE_LAG_RANGE = range(101, 199)
...
class NetboxDataSource:
        ...
        for nb_switch in nb_switches:
            ...
            for iface in nb_switch_detail.interfaces:
                ...
                if not far_device.cluster:
                    ...
                    continue
                if False and far_device.cluster.type.slug not in self.metagroup_handlers:
                    LOG.warning("Switch %s interface %s device %s cluster %s has unhandled cluster type %s",
                                nb_switch.name, iface.name, far_device.name, far_device.cluster.name)
                    continue
                ...
                # devices[far_device.id].ifaces.append({"name": iface.name, "lag": iface.lag.name, "lag_id": lag_id})
    ...
    def gen_direct_bindings_from_metagroup(self, metagroup: conf.Hostgroup) -> list[conf.Hostgroup]:
        return []

    def gen_direct_binding_hostgroup_from_device(self, device: Munch) -> conf.Hostgroup:
        return conf.Hostgroup(
            binding_hosts=[device.name],
            direct_binding=True,
            members=[],
        )
    ...
    def make_switchgroup(self, switchgroup_id: int, switches: list[Munch], asn_region: int,
        ...
        # asn / loopback
        use_ebgp = False  # FIXME: add as flag somewhere
        if use_ebgp:
    ...
    def generate_credentials(self, switchgroups: list[conf.SwitchGroup], user: str, password: str) -> conf.DriverCredentials:
        creds = {}
